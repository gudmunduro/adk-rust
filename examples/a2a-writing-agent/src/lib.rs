//! A2A v1.0.0 Writing Agent — LLM-powered article writing.
//!
...
pub const WRITING_SYSTEM_INSTRUCTION: &str = "\
You are a professional writer. When given a research summary or topic, produce a well-structured \
article with:\n\
\n\
1. **Introduction**: A compelling opening that introduces the topic and sets the context.\n\
2. **Body Sections**: Detailed, logically organized sections that expand on the key points, \
providing analysis, examples, and supporting evidence.\n\
3. **Conclusion**: A concise wrap-up that synthesizes the main takeaways and provides a \
forward-looking perspective.\n\
\n\
Write in a clear, engaging, and professional tone. Use headings, subheadings, and transitions \
to ensure the article flows naturally.";
...
pub fn build_writing_agent(
    model: Arc<dyn Llm>,
) -> Result<Arc<dyn Agent>, Box<dyn std::error::Error>> {
    let agent = LlmAgentBuilder::new("writing-agent")
        .description("Transforms research summaries into polished articles")
        .model(model)
        .instruction(WRITING_SYSTEM_INSTRUCTION)
        .build()
        .map_err(|e| format!("Failed to build writing agent: {e}"))?;
    Ok(Arc::new(agent))
}
...
pub fn build_server(agent: Arc<dyn Agent>, base_url: &str) -> Router {
    ...
    let card = build_v1_agent_card(
        "writing-agent",
        "A2A v1.0.0 LLM-powered writing agent that produces polished articles",
        &jsonrpc_url,
        "1.0.0",
        vec![AgentSkill {
            id: "write".to_string(),
            name: "Write Article".to_string(),
            description: "Transforms research summaries into polished articles".to_string(),
            tags: vec!["writing".to_string(), "article".to_string()],
            examples: None,
            input_modes: None,
            output_modes: None,
